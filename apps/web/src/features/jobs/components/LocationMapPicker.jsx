...
function LocationMapPicker({ location, onLocationChange, disabled = false }) {
  ...
  const chooseLocation = useCallback((point) => {
  ...
  useEffect(() => {
    ...
    async function initialize() {
      try {
        ...
        markerRef.current = new AdvancedMarkerElement({
          ...
          title: 'Miejsce wykonania usługi',
        })

        autocomplete = new PlaceAutocompleteElement()
        autocomplete.placeholder = 'Dokładny adres wykonania usługi'
        autocomplete.setAttribute('aria-label', 'Dokładny adres wykonania usługi')
    ...
    return () => {
      cancelled = true
      if (autocomplete && autocompleteHandler) autocomplete.removeEventListener('gmp-select', autocompleteHandler)
      autocomplete?.remove()
  ...
  useEffect(() => {
    if (!mapRef.current || !markerRef.current || mapsStatus !== 'ready') return
    markerRef.current.position = location ? toLatLng(location) : null
  }, [location, mapsStatus])
  ...
  function useCurrentLocation() {
    ...
    navigator.geolocation.getCurrentPosition(async (position) => {
      ...
      try {
        ...
        // Exact coordinates remain usable if reverse geocoding is temporarily unavailable.
      }
      chooseLocation({
        latitude: coordinates.lat,
        longitude: coordinates.lng,
  ...
  if (mapsStatus === 'unconfigured') {
    return <ManualLocationPicker location={location} onLocationChange={onLocationChange} />
  }
    ...
    <section className="route-picker">
      <div className="route-picker__inputs">
        <div className="route-picker__point route-picker__point--active">
          <span className="route-picker__badge">M</span>
          <div ref={autocompleteRef} className="route-picker__autocomplete" />
          {location && <small>{location.privateLabel}</small>}
        </div>
      </div>
...
function ManualLocationPicker({ location, onLocationChange }) {
  const current = location || { latitude: '', longitude: '', publicLabel: '', privateLabel: '', placeId: null }
  ...
  function change(field, rawValue) {
    ...
    if (Number.isFinite(latitude) && Number.isFinite(longitude) && next.publicLabel?.trim() && next.privateLabel?.trim()) {
      onLocationChange({ ...next, latitude, longitude, placeId: null })
    } else {
  ...
  return (
    <section className="route-picker route-picker--manual">
      <div className="route-picker__dev-note">
        ...
        <span>Dodaj VITE_GOOGLE_MAPS_BROWSER_KEY, aby dostać mapę, autocomplete i wybór miejsca kliknięciem.</span>
      </div>
      <fieldset className="route-picker__manual-point">
        <legend>Miejsce wykonania usługi</legend>
