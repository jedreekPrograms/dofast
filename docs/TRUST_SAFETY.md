...
The authenticated `/my-reports` page uses only `GET /job-reports/mine` and shows the reporter's own reason, optional note, submission time, coarse moderation status and the resolution timestamp once moderation reaches a terminal decision. It does not expose moderator identities, internal moderation notes, enforcement audits or another reporter's data. The page also keeps reports separate from payment/escrow disputes and directs transaction problems to the dedicated dispute workflow.
